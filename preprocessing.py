...
#  Get the dict maps each atom index to the mapping number.获得化合物中每个原子的索引，从1到n。
...
# Get the reactant atom index list in the order of product atom index.##统一order
...
def get_smarts_pieces(mol, src_adj, target_adj, reacts, add_bond=False):
    ...
    synthons = synthon_smiles.split('.')##合成子使用点号进行隔开
    # Find the reactant with maximum common atoms for each synthon
    syn_idx_list = [get_idx(synthon) for synthon in synthons]##合成子编为一个list
    react_idx_list = [get_idx(react) for react in reacts]##每个合成子对应的反应物编为一个list
...
def generate_opennmt_data(save_dir, set_name, data_files):
    ...
    for idx, f in tqdm(enumerate(data_files)):
        ...
        reactants = reactant.split('.') ##在数据集中反应试剂是以点号进行隔开的。
        ...
        tgt_data.append(tgt_item)##src是合成子；tgt是反应试剂。
...
def preprocess(save_dir, reactants, products, reaction_types=None):
    ...
    for index in tqdm(range(len(reactants))): 
        ...
        product_mol = Chem.MolFromSmiles(product)## 从smiles序列转化为分子图。
        reactant_mol = Chem.MolFromSmiles(reactant)

        product_adj = Chem.rdmolops.GetAdjacencyMatrix(product_mol)## 邻接矩阵
        product_adj = product_adj + np.eye(product_adj.shape[0])##主对角线上变为1
        product_adj = product_adj.astype(np.bool)##转化为bool类型
        ...
        patomidx2pmapidx = get_atomidx2mapidx(product_mol)##获得每个位置的索引 
        ...
        # full_order = order + growing_group_index0
        # reactant_adj = reactant_adj[full_order][:, full_order]

        product_bond_features = get_bond_features(product_mol) ##化学键的类型
        product_atom_features = get_atom_features(product_mol)## 原子的连接数，形式电荷，杂化方式等
        ## 产物和反应试剂进行统一的顺序编码。
        rxn_data = {
        ...
        with open(os.path.join(save_dir, 'rxn_data_{}.pkl'.format(index)),
                  'wb') as f:
            pickle.dump(rxn_data, f)  ##每个数据以pkl文件存储，每个数据是一个字典，
...
if __name__ == '__main__':
    ...
    for src, tgt in zip(sources, targets):
        ...
        reactants = tgt.split('.')# 合成子和最终的反应试剂是一一对应的。可能不同的合成子对应相同的反应试剂。
        if len(reactants) == 1:
            continue
        synthons.reverse()##为了进行数据增强，因为合成子之间并没有顺序。
        reactants.reverse()
